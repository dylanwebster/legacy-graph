...
| 5.2 | Map View (`/map`): bundled offline Natural Earth basemap (no third-party CDN), MapLibre + deck.gl event overlay with zoom-crossfaded heatmap → pins → focal path, dual-handle Radix time slider with step playback, scope filter (Everyone / Focal / Lineage) sharing `useFocalStore`, top-right collapsible event-type filter, deep-link contract round-trip, mobile 40 vh drawer with swipe-to-dismiss, LRU(32)-cached `GET /api/map/events` invalidated via `graph-updated`. See `SPECIFICATION.md §6.11`. |
