...
- [ ] **5.2.23** — Test coverage: ~~`tests/api/MapEvents.test.ts`~~ (Phase B: 8 tests including LRU cache + `graph-updated` invalidation + per-scope independence), ~~`client/src/features/map/timeStore.test.ts`~~ (Phase B: 9 tests covering granularity-sized seed, no-reset on subsequent calls, `isEventInWindow` cases), `client/src/features/map/layers/buildMapLayers.test.ts` (Phase C). *`styles.test.ts` shipped in A6 with 9 contract tests.*
