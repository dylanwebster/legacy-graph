...
- [x] **5.2.6a** — Time slider (single-handle, granularity, speed, loop, step-wise playback, keyboard, "Show undated"). *Superseded by 5.2.6b in Phase C.*
- [x] **5.2.6b** — Dual-handle time slider (Phase C). *Shipped: Radix `Slider` range mode with two thumbs, rAF batching during drag, authoritative commit on `onValueCommit`. Replaces 5.2.6a's single-handle. Folds in B6 (deferred from Phase B).*
...
- [x] **5.2.14** — Performance (Phase B): pre-jitter event positions on data arrival keyed on `dataUpdatedAt`; module-scope stable accessors in `buildMapLayers` (`getJitteredPosition`, `getRawPosition`, `getFillColor`, `getRadius`, `getWeight`); pre-filtered focal `personEvents` lifted into `MapView`; ref-keyed `fitBounds` guard (`${scope}:${focalPersonId}:${eventCount}`) so idempotent React Query refetches don't yank the user back; `GraphEngine` emits `graph-updated` on hydrate / `applyWriteSideEffects` / `removeNode` / story update / story delete + the `DELETE /api/people/:id` route emits as well; LRU(32) per-cache-key response memoization with generation guard in `src/api/routes/map.ts`. **B1 pre-bucketing dropped — overengineered for current scale**. **B6 (slider rAF batching + `pointerup` listener) deferred to Phase C5** to land alongside the Radix dual-handle replacement. **Zoom-lag fix**: `visible` (time-window-filtered events) lifted into a `useMemo` in `MapView` keyed on `(jitteredEvents, windowStart, windowEnd, showUndated)` so the array reference is stable across zoom changes and deck.gl skips GPU attribute regen; `buildMapLayers` always emits `events-heatmap` + `events-pins` and toggles deck.gl's `visible` prop instead of conditionally pushing — eliminates layer add/remove churn at the zoom-3 and zoom-5 crossfade thresholds (which previously tore down and rebuilt the HeatmapLayer's offscreen framebuffer on every gesture crossing them).
- [x] **5.2.15** — Event-type filter UI + collapsible `MapLegend` keyed by `TYPE_COLORS`. *Shipped in Phase C: `MapLegend.tsx` (top-right floating panel, collapsed by default) renders one row per `EVENT_TYPES` member with a color swatch (from `TYPE_COLORS`) and a checkbox. "All"/"None" quick buttons. Toggling collapses to `eventTypes: null` when every type is selected (URL-clean). Filter is applied alongside the time-window filter inside `MapView.visible` `useMemo` against a `Set<EventType>` for O(1) per-event cost. The focal `PathLayer` is intentionally unfiltered — the focal path is a structural feature, not a per-event view.*
- [x] **5.2.16** — Dual-handle range slider via Radix `Slider` range mode. *Shipped: see 5.2.6b.*
- [x] **5.2.17** — Empty-state and loading-state overlays; PathLayer 3 px stroke + 5 px halo. *Shipped in Phase C: `MapView` renders a translucent "Loading events…" pill while the React Query is `isLoading || (isFetching && !data)`, and a dismissible "No events with coordinates yet" card with a link to `/settings` when the response is `events.length === 0`. `buildMapLayers` now emits two stacked `PathLayer`s for focal scope: a 5 px halo (white at 70 % alpha for light theme, black at 55 % for dark) behind a 3 px stroke, drawn before the pin layer.*
- [x] **5.2.18** — Wire `?event=<id>` deep link to auto-open the EventDrawer. *Shipped in Phase C: a `useEffect` keyed on `(events.data, search.event)` opens the drawer when the URL value changes and the matching event exists. A `lastOpenedEventId` ref gates re-opens (closing the drawer clears the ref so the same id can be re-deep-linked from elsewhere).*
- [x] **5.2.19** — Mobile drawer: 40 vh bottom sheet (was 50 vh) + drag-down swipe handle. *Shipped in Phase C: `EventDrawer` height switched to `h-[40vh]` on mobile (desktop side panel unchanged). A drag handle bar uses Pointer Events to track downward drag — translateY mirrors the drag offset live, and on release a > 80 px drag dismisses; otherwise it springs back via the existing transition.*
- [x] **5.2.20** — Heatmap zoom-interpolated `radiusPixels` (30 → 60 px). *Shipped in Phase C: `MapView` derives `heatmapRadiusPixels = lerp(30, 60, clamp(zoom / 5))` in a `useMemo` keyed on the bucketed zoom state and threads it into `buildMapLayers` as a prop. The HeatmapLayer's `radiusPixels` is wired with an `updateTriggers` entry so deck.gl regenerates the framebuffer only when the bucketed value changes (a few times per gesture, not per frame).*
- [x] **5.2.21** — URL hygiene. *Shipped in Phase C: the debounced URL-writer effect now compares `Math.round(window) === Math.round(extent)` and omits `t`/`t_end` when they match — keeps the URL clean for users who haven't scrubbed. Drawer open/close routes through `onOpenEvent` / `onCloseDrawer` `useCallback`s that call `navigate({ search: prev => ({ ...prev, event: ... }) }, { replace: true })` synchronously so the URL is correct before the next debounced flush. Pin clicks go through a `clickRef` so layer rebuilds don't pay for navigate-callback identity changes.*
- [ ] **5.2.22** — Branch hygiene: squash three abandoned PMTiles commits, fold `MAP_VIEW.md` into `SPECIFICATION.md §6.11`. (Phase D)
- [x] **5.2.23** — Test coverage: ~~`tests/api/MapEvents.test.ts`~~ (Phase B: 8 tests including LRU cache + `graph-updated` invalidation + per-scope independence), ~~`client/src/features/map/timeStore.test.ts`~~ (Phase B: 9 tests covering granularity-sized seed, no-reset on subsequent calls, `isEventInWindow` cases), ~~`client/src/features/map/layers/buildMapLayers.test.ts`~~ (Phase C: 10 tests covering layer ids/visibility at zoom thresholds, crossfade band, heatmap radius pass-through, focal-path-halo + stroke ordering, halo-wider-than-stroke invariant, no focal-path when scope ≠ focal or fewer than 2 stops, layer-id stability across calls, jitter determinism). *`styles.test.ts` shipped in A6 with 9 contract tests.*
