...
- **Window-visibility filter**: per render, the visible event set is computed via a single linear `events.filter(...)` pass over the full prepared array (sized at ~10K in realistic data), comparing pre-parsed integer years — no string parsing on the hot path. Pre-bucketing by year was rejected as overengineering at this scale — the filter is sub-millisecond and trades simplicity for a marginal win that's invisible next to the GPU upload cost. Should the dataset grow past ~50K events, a sorted-by-`sort_date` array with a binary-search bracket is the next step (range events would need a separate IntervalTree-style index).
