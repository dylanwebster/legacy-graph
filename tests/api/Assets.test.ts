...
describe('Assets API', () => {
    ...
    it('GET /api/assets sets referencedBy.people correctly for person-owned asset', async () => {
        // Create a person with the test asset
    ...
    it('PUT /api/assets/:filename/meta updates caption', async () => {
        writeTestAsset();
        ...
        expect(Array.isArray(res.body.tagged_people)).toBe(true);

        // Verify persistence: assets.yaml written
        const metaYaml = path.join(META_DIR, 'assets.yaml');
        expect(fs.existsSync(metaYaml)).toBe(true);
    });

    it('PUT /api/assets/:filename/meta updates tagged_people', async () => {
        writeTestAsset();

        const res = await request
            .put(`/api/assets/${TEST_ASSET}/meta`)
            .send({ tagged_people: ['N_john-doe-1900-abc12345'] });

        expect(res.status).toBe(200);
        expect(res.body.tagged_people).toEqual(['N_john-doe-1900-abc12345']);
    });
