...
`;
        ...
        it('should parse ranges (BET/AND) into sort_date + sort_end_date', async () => {
            const input = `
0 HEAD
0 @I1@ INDI
1 NAME John /Doe/
...
`;
            ...
            expect(result.people[0].events[0].sort_end_date).toBe('1910-01-01');
            expect(result.people[0].events[0].end_date).toBe('1910');
        });
    });
